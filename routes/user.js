...
const addUser = (async (req, res, next) => {
  try {
    const insertSchema = Joi.object().keys({
      emp_name: Joi.string().alphanum().min(3).max(30)
        .required(),
...
const updateUser = (async (req, res, next) => {
  try {
    ...
    const updateUser = await poolCon.query(queryies.updateUser, [req.body.emp_name, req.params.id]);
...
const deleteUser = (async (req, res, next) => {
  try {
    const deleteUser = await poolCon.query(queryies.deleteUser, [false, req.params.id]);
